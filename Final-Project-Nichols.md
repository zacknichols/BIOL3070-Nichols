...
treatment and harvest yield categories, a boxplot with overlaid jittered
observations was used. This graphical analysis allows for inspection of
group-level patterns, such as differences in the median, variability,
and overall distribution of categorical harvest scores across both
pruned and unpruned trees. Although the response variable is ordinal,
the boxplot provides an intuitive perspective of whether pruned trees
tend to receive higher rating or showed less variability when compared
to unpruned individuals. This visualization serves as a complementary,
non-statistical tool for assessing trend that could support or
contextualize the results of the previous chi-squared test.

``` r
ggplot(pruning_data, aes(x=pruningstatus, y=sizeofharvest)) + 
  # point parameters
  geom_point(size=3, fill="gray50", alpha=.2) + # box point parameters
  geom_jitter(color="grey50", width=.2, size=0.5, alpha=0.4) + # individual data points
  # box parameters
  ...
  theme(
    plot.title = element_text(hjust=0.5, size=20, face="bold"), # changes title format
...
The boxplot provided a visual comparison of yield score distributions
across pruned and unpruned individuals. Pruned and unpruned trees showed
majority overlapping distributions, with similar medians and variability
in harvest scores between both treatments. While pruned trees display a
slightly higher central tendency according to the jitters of individual
observations, the difference is small and deemed moot by the substantial
overlap between the two treatments. Overall, the boxplot supports the
chi-square test’s findings by illustrating that pruning practice did not
produce a clear or consistent difference in categorical harvest-yeild
outcomes.
